...
allMeals(AllergyList,List)  :-
       findall(X,(findfood(AllergyList,Y),meal(X,Z,_,_),member(Y,Z)),L),removeDups(L,List).
...
allLikedMeals(Like,Meal)  :-
     findall(A,(meal(A,X,_,_),member(B,Like),member(B,X)),Meal).
...
calorieLimit(Meal,CalorieLimit)      :-
      CalorieLimit>0,meal(Meal,_,X,_), CalorieLimit<X.
...
findMealsForTime(X,Initial,MealList)  :-
     findall(A,( member(A,Initial), meal(A,_,_,B), B=<X ),MealList).
...
orderLikedList(LikeList,Initial,MealList)  :-
 ...
 %all meals
allMeals(X)  :-
     findall(A,meal(A,_,_,_),X).


%unites all predicates and creates personallist
listPersonalList(User,PersonalList)   :-
     user(User,Allergy,Type,Dislikes,Likes,Time),
     allMeals(A),
     ...
     findMealsForTime(Time,F,PersonalList),
     write(PersonalList).
